...
Page({
  ...
  onLoad: function () {
    ...
    if (nameListArr) {
      ...
      if (nameListArr.length > 1) {
        this.setData({
          name: nameListArr[nameListArr.length - 1]
        })
      }
    } else {
  ...
  init() {
    ...
    ctx.drawImage("../../images/house2.jpg", 0, 0, this.data.canvasWidth, this.data.canvasHeight);
    if(this.data.isSetTx){
    ...
    ctx.fillText(this.data.name, this.data.canvasWidth / 1.42, this.data.canvasWidth / 1.36)

    ctx.font = "bold 18px Arial";
    ctx.fillStyle = "#000"
    ctx.setTextAlign("center")
    ctx.fillText(this.data.project1, this.data.canvasWidth / 1.8, this.data.canvasWidth / 1.205)

    ctx.font = "16px Arial";
    ctx.fillStyle = "#000"
    ctx.setTextAlign("left")
    this.drawText(ctx, this.data.title2, 50, this.data.canvasWidth / 1.040, 230 / 375 * this.data.canvasWidth)
    // ctx.fillText()
      ...
      success: function (res) {
        ...
        if (res.authSetting['scope.writePhotosAlbum'] == false) {
          ...
          if (!me.data.isSeeOnce) {
            const videoAd = wx.createRewardedVideoAd({
              adUnitId: 'adunit-9daca53b0aa9b4b5'
            })
            videoAd.onError(err => {
